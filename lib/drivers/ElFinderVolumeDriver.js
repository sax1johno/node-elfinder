...
var ElFinderVolumeDriver = function() {
    /**
	 ...
	 * @var string
	 **/
	var separator = DIRECTORY_SEPARATOR;
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.configure = function () {
	// set thumbnails path
	var path = this.options.tmbPath
	if (path) {
		if (!this.file_exists(path)) {
			if (this.mkdir(path)) {
				this.chmod(path, options.tmbPathMode);
			} else {
				path = "";
			}
		} 
		...
		if (this.is_dir(path) && this.is_readable(path)) {
			this.tmbPath = path;
		    this.tmbPathWritable = this.is_writable(path);
		}
	}

	// set image manipulation library
	var type = options.imgLib.match('/^(imagick|gd|auto)$/i')
		? options.imgLib.toLowerCase()
		: 'auto';
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.driverId = function() {
	return driverId;
};
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.id = function() {
	return id;
};
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.debug = function() {
	return {
		'id'         : id,
		'name'       : driverName,
		'mimeDetect' : mimeDetect,
		'imgLib'     : imgLib
	};
}
 ...
 **/
ElFinderVolumeDriver.prototype.mount(opts) {
    var mountDeferred = Q.defer();
    if (_.isUndefined(opts.path) || opts.path === '') {
        mountDeferred.reject(this.setError('Path undefined'));
    }

    options = _.extend(options, opts);
    id = driverId + (!_.isUndefined(options.id)) ? options.id : this.context.volumesCount;
    root = this._normPath(options.path);
    separator = !_.isUndefined(separator) ? options.separator : DIRECTORY_SEPARATOR;

    defaults = {
        'read': (options.defaults.read ? !!options.defaults.read : true),
        'write': (options.defaults.write ? !!options.defaults.write : true),
        'locked': false,
        'hidden': false
    }

    attributes.push({
        'pattern': '~^' + escapeRegExp(DIRECTORY_SEPARATOR) + '$~',
        'locked': true,
        'hidden': false
    });
    ...
    Q.fcall(function() {
        ...
        if (!_.isUndefined(options.attributes) && _.isArray(options.attributes)) {
            _.each(options.attributes, function(attr, index, list) {
                if (!_.isUndefined(attr.pattern) && attr.length > 1) {
                    attributes.push(a);
                }
        ...
        today     = mktime(0,0,0, Date.getMonth(), Date.getDay(), Date.getYear());
    	yesterday = today-86400;
    
    	// debug($this->attributes);
    	if (!this.init()) {
    		mountDeferred.reject();
    	}
        
        // check some options is arrays
        uploadAllow = !_.isUndefined(options.uploadAllow) && !_.isArray(options.uploadAllow)
            ? options.uploadAllow
    		: [];
    
    	uploadDeny = !_.isUndefined(options.uploadDeny) && _.isArray(options.uploadDeny)
    		? options.uploadDeny
    		: [];
    
    	uploadOrder = options.uploadOrder; // This driver doesn't supported telephat mode.
    	...
    	var type = options.mimeDetect.toLowerCase();
    	type = type.match('/^(finfo|mime_content_type|internal|auto)$/i') ? type : 'auto';
    	var regex = '/text\/x\-(php|c\+\+)/';

    	rootName = _.isUndefined(options.alias) ? this.basename(root) : options.alias;
    	root = this.stat(root);
    
    	if (!root) {
            return Q.reject(this.setError('Root folder does not exist.'));
    	}
        if (!root.read && !root.write) {
            return Q.reject(this.setError('Root folder does not have read and write permissions.'));
        }
        ...
        if (root.read) {
            if (options.startPath) {
                ...
                this._inpath(options.startPath, root)) {
                    ...
                    if (_.last(startPath) == options.separator) {
        ...
        treeDeep = options.treeDeep > 0 ? options.treeDeep + 0 : 1;
        tmbSize = options.tmbSize > 0 ? options.tmbSize + 0: 48;
        URL = options.URL;
        if (URL && URL.match("|[^/?&=]$|")) {
            URL += '/';   
        }
        ...
        nameValidator = _.isString(options.acceptedName) && !_.isUndefined(options.acceptedName) ?
        ...
        if (!_.isUndefined(options.archiveMimes) && !_.isArray(options.archiveMimes)) {
            var index = 0;
            _.each(archivers.create, function(value, key, list) {
                if (!_.contains(key, _.keys(options.archiveMimes))) {
                    delete archivers.create[key];
                }
                if (++index > _.size(list) - 1) {
                    return Q.resolve();
                }
        ...
        if (options.archivers.create && _.isArray(options.archivers.create)) {
            var index = 0;
            _.each(options.archivers.create, function(conf, mime, list) {
                if (mime.indexOf('application/') ===  0
                    && !_.isUndefined(conf.cmd)
                    && !_.isUndefined(conf.argc)
                    && !_.isUndefined(conf.ext)
                    && !_.isUndefined(archivers.create[mime])) {
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.umount = function() {
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.error = function() {
	return error;
}
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.setMimesFilter = function(mimes) {
	if (_.isArray(mimes)) {
		onlyMimes = $mimes;
	}
};
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.root = function() {
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.defaultPath = function() {
	return this.encode(startPath ? startPath : root);
}
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.options(hash) {
	return {
		...
		'disabled'      : disabled,
		'separator'     : separator,
		'copyOverwrite' : options.copyOverwrite,
		'archivers'     : {
			'create'  : archivers.create ? _.keys(archivers.create) : [],
			'extract' : archivers.extract ? _.keys(archivers.extract) : []
		}
	};
}
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.commandDisabled = function(cmd) {
    return _.contains(cmd, disabled);
};
 ...
 **/
ElFinderVolumeDriver.prototype.mimeAccepted = function(mime, mimes, empty) {
    mimes = !_.isEmpty(mimes) ? mimes : onlyMimes;
	...
	return mime == 'directory'
		|| _.contains('all', mimes)
		|| _.contains('All', mimes)
		|| _.contains(mime, mimes)
		|| _.contains(mime.substr(0, mime.indexOf('/')), mimes);
};
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.isReadable = function() {
	var stat = this.stat(root);
	return stat.read;
};
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.copyFromAllowed = function() {
	return !!options.copyFrom;
};
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.path = function(hash) {
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.realpath = function(hash) {
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.removed = function() {
	return this.removed;
};
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.resetRemoved = function() {
	this.removed = [];
};
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.closest = function(hash, attr, val) {
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.file(hash) {
	path = this.decode(hash);

	return (file = this.stat(path)) ? file : this.setError(ERRORS.ERROR_FILE_NOT_FOUND);
	...
	return this.setError(ERRORS.ERROR_FILE_NOT_FOUND);
}
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.dir(hash, resolveLink) {
    resolveLink = false;
    var dir;
	if ((dir = this.file(hash)) === false) {
		return this.setError(ERRORS.ERROR_DIR_NOT_FOUND);
	}

	if (resolveLink && !_.isUndefined(dir.thash)) {
		dir = this.file(dir.thash);
	}

	return dir && dir.mime == 'directory' && !dir.hidden
		? dir 
		: this.setError(ERRORS.ERROR_NOT_DIR);
}
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.scandir = function(hash) {
    var dir;
	...
	return dir.read
		...
		: this.setError(ERRORS.ERROR_PERM_DENIED);
};
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.ls = function(hash) {
    var lsDefer = Q.defer();
    var dir;
	...
	var ls = [];
	var path = this.decode(hash);
    
    _.each(this.getScanDir(path), function(stat, index, list) {
        if (!stat.hidden && this.mimeAccepted(stat.mime)) {
            ls.push(stat.name);
        }
        if (index > _.size(list) - 1) {
            lsDefer.resolve(ls);
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.tree = function(hash, deep, exclude) {
    var hash = '';
    var deep = 0;
    var exclude = '';
	var path = hash ? this.decode(hash) : this.root;
    var dir;
    var dirs;
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.parents = function(hash) {
	...
	while (path && path !== this.root) {
		...
		tree.unshift(stat);
		if (path !== this.root) {
            this.gettree(path, 0).then(function(thisTree) {
                _.each(thisTree, function(dir, index, list) {
                    if (!_.contains(dir, tree)) {
                        tree.push(dir);
                    }
                    if (index > _.size(list) - 1) {
                        parentsDefer.resolve(tree ? tree : current);
                    }
                });
            });
		}
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.tmb = function(hash) {
    var tmbDefer = Q.defer();
	var path = this.decode(hash);
	var stat = this.stat(path);

    if (!stat.tmb) {
        tmbDefer.resolve(stat.tmb == "1" ? this.createTmb(path, start) : stat.tmb);
    } else {
        tmbDefer.reject();
    }
};
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.size = function(hash) {
 ...
 * @return Resource
 **/
ElFinderVolumeDriver.prototype.open = function(hash) {
    var file;
 ...
 * @return void
 **/
ElFinderVolumeDriver.prototype.close = function(fp, hash) {
	return this._fclose(fp, this.decode(hash));
}
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.mkdir = function(dst, name) {
	if (this.commandDisabled('mkdir')) {
		return this.setError(ERRORS.ERROR_PERM_DENIED);
	}

	if (!this.nameAccepted(name)) {
		return this.setError(ERRORS.ERROR_INVALID_NAME);
	}

    var dir;
	if ((dir = this.dir(dst)) === false) {
		return this.setError(ERRORS.ERROR_TRGDIR_NOT_FOUND, '#'+dst);
	}
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.mkfile(dst, name) {
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.rename = function(hash, name) {
    var renameDefer = Q.defer();
	...
	if (!this.nameAccepted(name)) {
		return this.setError(ERRORS.ERROR_INVALID_NAME, name);
	}
    
    var file;
    var dir;
    this.file(hash).then(function(file) {
        if (!file) {
            return renameDefer.reject(this.setError(ERRORS.ERROR_FILE_NOT_FOUND));
        }
    }).then(function() {
        if (name == file.name) {
            return renameDefer.resolve(file);
        }
        if (!file.locked) {
            return renameDefer.reject(this.setError(ERRORS.ERROR_LOCKED, file.name));
        }
        var path = this.decode(hash);
        return Q.resolve(path);
    }).then(function(path) {
        return this._dirname(path).then(function(thisDir) {
            dir = thisDir;
            return Q.resolve(dir);
        });
    })
    .then(function(dir) {
        var pth = this._joinPath(dir, name);
        return this.stat(pth);
    }).then(function(stat) {
        if (stat) {
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.duplicate = function(hash, suffix) {
    var duplicateDefer = Q.defer();
    var path;
    var dir;
    var name;
    if (!suffix) {
	...
	if (this.commandDisabled('duplicate')) {
		return this.setError(ERRORS.ERROR_COPY, '#'+hash, ERRORS.ERROR_PERM_DENIED);
	}
    var file;
    this.file(hash).then(function(file) {
        if (file === false) {
    ...
    .then(function(nme) {
        ...
        if (!this.allowCreate(dir, name)) {
          return duplicateDefer.reject(this.setError(ERRORS.ERROR_PERM_DENIED));
        }
    ...
    .then(function(stat) {
        duplicateDefer.resolve(stat)
    }, function() {
        duplicateDefer.reject();
    })

	return duplicateDefer.promise;
}
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.upload = function(fp, dst, name, tmpname) {
    ...
    }).then(this.nameAccepted(name))
    .then(function(accepted) {
        if (!accepted) {
            return uploadDefer.reject(this.setError(ERRORS.ERROR_INVALID_NAME));
        }
        return Q.resolve();
    }).then(this.mimeDetect())
    .then(function(detect) {
        return this.mimetype(detect == 'internal' ? name : tmpname, name);
    }).then(function(mime) {
        ...
        upload = true;
        if(this.uploadOrder[0].toLowerCase() == 'allow') {
            upload = false;
            if (!deny && allow) {
                upload = true;
            }
    ...
    .then(function(file) {
        ...
        var s = getimagesize(tmpname);
        if (mime.indexOf('image') === 0 && s) {
            stat.width = s[0];
            stat.height = s[1];
        }
        return uploadDefer.resolve();
    }).then(this._save(fp, dstpath, name, stat))
    .then(function(path) {
        return uploadDefer.resolve(this.stat(path));
    }, function() {
        return uploadDefer.reject();
    });
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.paste = function(vol, src, dst, rmSrc) {
    var pasteDefer = Q.defer();
    rmSrc = false;
	var err = rmSrc ? ERRORS.ERROR_MOVE : ERRORS.ERROR_COPY;

	if (this.commandDisabled('paste')) {
        return pasteDefer.reject(this.setError(err, '#'+src, ERRORS.ERROR_PERM_DENIED));
	}
    var name;
    var errpath;
    var dir;
    var destination;
    var source;
    vol.file(src, rmSrc).then(function(file) {
        if (!file) {
    ...
    }).then(vol.closest(src, rmsrc ? 'locked' : 'read', rmSrc))
    .then(function(tst) {
        test = tst;
        if (test) {
            if (rmSrc) {
    ...
    .then(function(st) {
        ...
        if (stat) {
            if (this.options.copyOverwrite) {
                ...
                if (!stat.write) {
                    return pasteDefer.reject(err, errpath, ERRORS.ERROR_PERM_DENIED);
                }
            }
            return Q.resolve();
        } else {
    ...
    .then(function(locked) {
        ...
        vol = this && (test == file.target) || test == this.decode(src);
        if (vol) {
            return pasteDefer.reject(ERRORS.ERROR_REPLACE, errpath);   
        }
    }).then(this.remove(test))
    .then(function(removed) {
        ...
        if (vol == this) {
            ...
            return Q.resolve(path = this.method(source, destination, name)) ? this.stat(path) : false;
        }
    }).then(function() {
        if (!this.options.copyTo || !vol.copyFromAllowed()) {
            return Q.reject(this.setError(ERRORS.ERROR_COPY, errpath, ERRORS.ERROR_PERM_DENIED));
        }
        if ((path = this.copyFrom(vol, src, destination, name)) === false) {
            return Q.reject();
        }
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.getContents = function(hash) {
    var getContentsDefer = Q.defer;
    var file;
    ...
    this.file(hash).then(function() {
        ...
        if (!file) {
            return getContentsDefer.reject(this.setError(ERRORS.ERROR_FILE_NOT_FOUND));
        }
        if (file.mime == 'directory') {
            return getContentsDefer.reject(this.setError(ERRORS.ERROR_NOT_FILE));
        }
    ...
    return getContentsDefer.promise;
}
 ...
 * @author Dmitry (dio) Levashov
 **/
ElFinderVolumeDriver.prototype.putContents = function(hash, content) {
    var putContentsDeferred = Q.defer();
    var path;
    
    Q.fcall(function() {
        if (this.commandDisabled('edit')) {
            return putContentsDeferred.reject(this.setError(ERRORS.ERROR_PERM_DENIED))l
        }
        path = this.decode(hash);
    }).then(this.file(hash)).then(function(file) {
        if (!file) {
            return putContentsDeferred.reject(this.setError(ERRORS.ERROR_FILE_NOT_FOUND));
        }
        if (!file.write) {
            return putContentsDeferred.reject(this.setError(ERRORS.ERROR_PERM_DENIED));
        }
        this.clearCache();
    ...
    return putContentsDeferred.promise;
}
 ...
 * @author Alexey Sukhotin
 **/
ElFinderVolumeDriver.prototype.extract = function(hash) {
    ...
    .then(function(fle) {
        ...
        archiver = !_.isUndefined(this.archivers.extract[file.mime])
    ...
    .then(function(stat) {
         path = stat;
         return extractDeferred.resolve(path);
    }, function() {
        path = false;
        return extractDeferred.reject();
    })
 ...
 * @author Dmitry(dio) Levashov
 **/
ElFinderVolumeDriver.prototype.setError = function(error) {
	this.errors = [];
    if (!_.isArray(error)) {
        this.errors.push(error);
    } else {
        _.flatten(this.errors.join(error));
    }
    return this.errors;
}
...
module.exports = ElFinderVolumeDriver;